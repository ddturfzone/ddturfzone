...
// BUG FIX — admin.html calls window.getFirebaseStatus() in ~8 places
// (the reconnect watchdog, Diagnostics page, enrolment approve/reject
// guards) but this function never existed here. The watchdog in
// particular always read `status.fullyReady` as undefined and returned
// early on every single trigger (tab-focus, online event, reconnect),
// meaning dropped Firebase listeners were NEVER actually restored after
// a reconnect — this exposes the exact live internal state the rest of
// firebase.js already tracks (connectionStatus/firebaseFullyReady/
// authState/lastFirebaseError), read-only, no side effects.
...
// BUG FIX — admin.html's reconnect watchdog calls this after every
// reconnect/tab-focus/online event to re-assert the topbar indicator
// from live state (in case `.info/connected` didn't fire a fresh
// change event); it never existed, so the indicator could stay stuck
// on a stale state. Just exposes the existing internal refresh.
window.refreshFirebaseConnectionIndicator = () => _updateConnectionIndicator();

// BUG FIX — admin.html calls this once after startup to log whether
// the connection is genuinely live; never existed (silently skipped
// via optional chaining, so no crash, just a missing log line). A
// one-shot, read-only check of the same '.info/connected' node the
// ongoing monitor above already watches.
window.verifyFirebaseConnection = async () => {
  try {
    if (!realtimeDb) return { success: false, message: 'Realtime Database not initialized' };
    const snap = await firebase.database(firebaseApp).ref('.info/connected').once('value');
